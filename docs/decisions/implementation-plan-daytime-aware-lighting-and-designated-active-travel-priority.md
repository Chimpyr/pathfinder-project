...
- Neutral lighting multipliers in daylight for `prefer_lit` and `heavily_avoid_unlit`.
...
| R2  | Daylight neutralization for lighting penalties           | With `lighting_context=daylight`, `prefer_lit` and `heavily_avoid_unlit` both result in effective multiplier `1.0` for all edges.                                                   |
...
| R9  | Way-specific fixture is preserved under relevant toggles | Fixture route containing OSM Way `1472097444` remains competitive/preferred with `prefer_paved + avoid_unsafe_roads`, and is not penalised by lighting toggles in daylight context. |
...
| IT-03   | R9           | Integration | Fixture route including Way `1472097444` with `prefer_paved + avoid_unsafe_roads`      | Route remains competitive/preferred over unsafe or unpaved alternatives.    |
