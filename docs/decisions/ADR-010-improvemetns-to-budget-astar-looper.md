...
**Status: PROPOSED** — requires data pipeline change (tile cache
invalidation) and solver update. The cycleway exclusion fix should be
implemented first as it is a data availability bug rather than a preference
tuning issue.
...
### Backend changes (§2a — PROPOSED, separate effort)

- **`data_loader.py`**: Add `'designation'` to `extra_attributes`. Add
  supplementary cycleway query to include `highway=cycleway` + `foot ∈
{yes, designated}` ways in the walking graph.
- **`budget_astar_solver.py`**: Extend `_road_type_penalty()` to inspect
  `foot` and `designation` tags, applying a bonus multiplier (< 1.0) for
  designated footpaths/bridleways when `prefer_pedestrian` is enabled.
- **Tile cache**: Existing caches must be invalidated after the data pipeline
  change so that shared-use cycleways are included in rebuilt graphs.
