...
export function pollTask(
  ...
  onSuccess,
  onError,
  maxTimeMs = CONFIG.MAX_POLL_TIME_MS,
) {
  ...
  const checkStatus = async () => {
    try {
      ...
      // Support both legacy {state: SUCCESS/FAILURE} and current
      // {status: complete/failed/building/pending} task payloads.
      const rawStatus = String(data.status || data.state || "").toLowerCase();
      const isComplete = rawStatus === "complete" || rawStatus === "success";
      const isFailed =
        rawStatus === "failed" ||
        rawStatus === "failure" ||
        rawStatus === "revoked";
      ...
      if (isComplete) {
        ...
        onError(data.error || data.status || "Task failed");
      } else {
