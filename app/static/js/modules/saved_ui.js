...
export function initSavedUI() {
  ...
  function renderPins(pins) {
    ...
    pins.forEach((pin) => {
      ...
      card.addEventListener("mouseenter", () => {
        if (mapController) {
          mapController.showTempPinMarker(
            pin.latitude,
            pin.longitude,
            pin.label,
          );
      ...
      card.addEventListener("click", (e) => {
        ...
        if (
          e.target.closest(".saved-card-delete") ||
          e.target.closest(".saved-card-rename") ||
          e.target.closest(".saved-pin-route-btn")
        )
          return;
      ...
      // Delete button
      card
        .querySelector(".saved-card-delete")
      ...
      // Rename button
      card
        .querySelector(".saved-card-rename")
  ...
  function startRename(card, pin) {
    ...
    actions
      .querySelector(".saved-rename-confirm")
      .addEventListener("click", (e) => {
        e.stopPropagation();
        finishRename(true);
      });
    actions
      .querySelector(".saved-rename-cancel")
      .addEventListener("click", (e) => {
        e.stopPropagation();
        finishRename(false);
      });
  ...
  function renderQueries(queries) {
    ...
    queries.forEach((query) => {
      ...
      // Format distance
      const distStr = query.distance_km
        ? `${query.distance_km.toFixed(1)} km`
        : "—";

      // Build start/end display
      const startStr = `${query.start_lat.toFixed(4)}, ${query.start_lon.toFixed(4)}`;
      const endStr =
        query.end_lat != null && query.end_lon != null
  ...
  function loadQueryIntoFinder(query) {
    ...
    if (query.weights && typeof query.weights === "object") {
      ...
      if (
        ...
        const legacyToDedicated = document.getElementById(
  ...
  function buildWeightPills(weights) {
    ...
    const preferences = [
      ...
      {
        key: "prefer_nature_trails",
        icon: "🌲",
        name: "Nature trails",
        type: "toggle",
      },
      {
        key: "prefer_pedestrian",
        icon: "🚶",
    ...
    for (const pref of preferences) {
      ...
      if (pref.type === "toggle") {
        ...
        if (val === true) {
          pills.push(
            `<span class="weight-pill weight-pill-on">${pref.icon} ${pref.name}</span>`,
          );
        }
      } else {
        ...
        if (num > 0) {
          ...
          pills.push(
            `<span class="weight-pill">${pref.icon} ${pref.name}: ${level}</span>`,
          );
        }
      }
    }
