 ...
 */
import { appState, startState, endState, routeState, loopState, setRoutingMode, setSelectedDirection } from './state.js';
import { createRouteTask, createLoopTask, pollTask, submitFeedback } from './api.js';
...
const preferPedestrianToggle = document.getElementById("prefer-pedestrian-toggle");
...
function initModeToggles() {
    if (modeStandardBtn) modeStandardBtn.addEventListener("click", () => switchRoutingMode("standard"));
    if (modeLoopBtn) modeLoopBtn.addEventListener("click", () => switchRoutingMode("loop"));
}
...
function switchRoutingMode(mode) {
    ...
    if (endLocationGroup) endLocationGroup.classList.toggle("hidden", !isStandard);
    if (loopDistanceGroup) loopDistanceGroup.classList.toggle("hidden", isStandard);
...
function updateLoopDistanceWarning(km) {
    ...
    if (km > 25) {
        longLoopWarning.className = "p-3 rounded-lg border transition-all loop-warning-red";
        longLoopWarningText.innerHTML = "<strong>Very long route!</strong> Distances over 25 km will take significantly longer and may timeout.";
    } else if (km > 20) {
        longLoopWarning.className = "p-3 rounded-lg border transition-all loop-warning-orange";
        longLoopWarningText.innerHTML = "<strong>Long route.</strong> Distances over 20 km may take a moment to calculate.";
    } else if (km > 15) {
        longLoopWarning.className = "p-3 rounded-lg border transition-all loop-warning-amber";
        longLoopWarningText.textContent = "Routes over 15 km may take a moment to calculate.";
    } else {
...
async function handleStandardSubmit() {
    ...
    if (preferPedestrianToggle && preferPedestrianToggle.checked) payload.prefer_pedestrian = true;
    if (preferPavedToggle && preferPavedToggle.checked) payload.prefer_paved = true;
    if (preferLitToggle && preferLitToggle.checked) payload.prefer_lit = true;
    if (avoidUnsafeToggle && avoidUnsafeToggle.checked) payload.avoid_unsafe_roads = true;
...
async function handleLoopSubmit() {
    ...
    const payload = {
        ...
        prefer_pedestrian: preferPedestrianToggle ? preferPedestrianToggle.checked : false,
...
function onRouteSuccess(result) {
    ...
    // Draw on map
    if (mapController) {
        console.log("[RoutingUI] Calling mapController.displayMultipleRoutes with:", result.routes);
